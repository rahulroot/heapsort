...
/* Function is ugly, need to modify */
int find_max(int lc, int rc, int pos)
{
	int lmax = pos;
	if( (lc <= maxnum))
	{
...
{
#if 0
	cout <<"Heapifying for index : " << pos << endl;
#endif
	int lc = 2*pos;
	int rc = 2*pos + 1;
	int max = find_max(lc, rc, pos);
#if 0
	if( (lc <= maxnum) && ( a[lc] > a[pos]))
		max_1 = lc;
	else
		max = pos;

	if( (rc <= maxnum) && ( a[rc] > a[pos]))
		max = rc;
#endif
